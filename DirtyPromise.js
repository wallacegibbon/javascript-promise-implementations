...
export default class DirtyPromise {
  constructor(fn) {
    if (typeof this !== "object") throw new TypeError('Promise must be constructed with keyword "new"');
    if (!is_function(fn)) throw new TypeError("constructor argument have to be a function");
  ...
  static all(promise_array) {
    if (!Array.isArray(promise_array)) throw new TypeError("Promise.all need Array object as argument");
  ...
  static race(promise_array) {
    if (!Array.isArray(promise_array)) throw new TypeError("Promise.race need Array object as argument");
    return new DirtyPromise((res, rej) => promise_array.forEach(p => p.then(res, rej)));
  }

  static any(promise_array) {
    if (!Array.isArray(promise_array)) throw new TypeError("Promise.any need Array object as argument");
